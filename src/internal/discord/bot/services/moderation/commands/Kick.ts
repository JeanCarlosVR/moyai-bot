...
export default class extends Command {
    ...
    public async run() {
        ...
        if(!this.message.channel.guild.me.permission.has("kickMembers")){
            return this.send({
                  description: `I not have \`Kick Members\` permissions. Please re-invite me.`
            });
        }
      
        if (!(this.message.member.permission.has("kickMembers"))) {
            return this.send({
                description: "You not have `Kick Members` permissions."
            });
        }
        ...
        if(!(this.message.member.id === this.message.channel.guild.ownerID || this.message.member.permission.has("administrator"))) {
            if (!this.message.member.highestRole.higherThan(member.highestRole)) {
                return this.send({
                    description: "You cannot kick a member with a higher role than yours."
                });
            }
      
            if (this.message.member.highestRole.position >= member.highestRole.position) {
                return this.send({
                    description: "You cannot kick a member with a higher or equal role than yours."
                });
            }
        }
        ...
        if(!member.punishable) {
            return this.send({
                description: "This member is not kickeable."
            });
        }
